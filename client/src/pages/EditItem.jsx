      ...
      <div className="bg-white border-b border-gray-200 sticky top-16 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-4">
             <button onClick={() => navigate(-1)} className="text-gray-500 hover:text-indigo-600 transition text-xl">
                <FaArrowLeft />
             </button>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    {error && (
                        <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-md">
                            <p className="text-red-700 text-sm font-bold italic">⚠️ {error}</p>
                        </div>
                    )}

                    {/* Media Section */}
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Photos (Max 3)</h3>
                        <div className="grid grid-cols-3 gap-4 mb-6">
                            {previews.map((src, index) => (
                                <div key={index} className="relative h-28 rounded-xl overflow-hidden border border-gray-100 group shadow-sm">
                            ...
                            {previews.length < 3 && (
                                <label className="h-28 border-2 border-dashed border-indigo-100 rounded-xl flex flex-col items-center justify-center bg-indigo-50/30 text-indigo-400 cursor-pointer hover:bg-indigo-50 transition">
                                    <FaCamera size={24} />
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div className="mt-1 relative rounded-xl shadow-sm">
                                    ...
                                    <input type="number" name="price" required value={formData.price} onChange={handleChange} className="block w-full pl-10 bg-gray-50 border-gray-200 focus:bg-white focus:ring-indigo-500 rounded-xl py-3" />
                                </div>
                            </div>

                            <div>
                                <label className="block text-sm font-bold text-gray-700">Category</label>
                                <select name="category" value={formData.category} onChange={handleChange} className="mt-1 block w-full bg-gray-50 border-gray-200 focus:bg-white focus:ring-indigo-500 rounded-xl px-4 py-3">
                            ...
                            {formData.category === 'Others' && (
                                <div className="md:col-span-2">
                                     <label className="block text-sm font-bold text-gray-700">Specify Category</label>
                                     <input type="text" name="customCategory" required value={formData.customCategory} onChange={handleChange} className="mt-1 block w-full bg-gray-50 border-gray-200 focus:bg-white focus:ring-indigo-500 rounded-xl px-4 py-3" placeholder="e.g. Sports Gear" />
                                </div>
                            )}
                    ...
                    {/* Contact Info */}
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Contact & Location</h3>
                        <div className="grid grid-cols-1 gap-4">
                            <div>
                                <label className="block text-sm font-bold text-gray-700">Hostel / Location</label>
                                <div className="mt-1 relative rounded-lg">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <FaMapMarkerAlt className="text-gray-400" />
                                    </div>
                                    <input type="text" name="location" required value={formData.location} onChange={handleChange} className="block w-full pl-10 bg-gray-50 border-gray-200 focus:bg-white focus:ring-indigo-500 rounded-xl py-3" />
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="relative">
                                    <FaUser className="absolute top-4 left-3 text-gray-400" />
                                    <input type="text" name="sellerName" required value={formData.sellerName} onChange={handleChange} className="block w-full pl-10 bg-gray-50 border-gray-200 focus:bg-white focus:ring-indigo-500 rounded-xl py-3" placeholder="Seller Name" />
                                </div>
                                <div className="relative">
                                    <FaPhone className="absolute top-4 left-3 text-gray-400" />
                                    <input type="tel" name="sellerPhone" required maxLength="10" value={formData.sellerPhone} onChange={handleChange} className="block w-full pl-10 bg-gray-50 border-gray-200 focus:bg-white focus:ring-indigo-500 rounded-xl py-3" placeholder="Phone (10 digits)" />
                                </div>
                            </div>
                            <div className="relative">
                                <FaEnvelope className="absolute top-4 left-3 text-gray-400" />
                                <input type="email" name="sellerEmail" required value={formData.sellerEmail} onChange={handleChange} className="block w-full pl-10 bg-gray-50 border-gray-200 focus:bg-white focus:ring-indigo-500 rounded-xl py-3" placeholder="Email Address" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-end space-x-4 pt-4">
                        <button type="button" onClick={() => navigate('/my-listings')} className="px-6 py-3 font-bold text-gray-500 hover:text-gray-800 transition">Discard</button>
                        <button type="submit" disabled={submitting} className="px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg flex items-center disabled:bg-gray-400">
                ...
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                    ...
                    <div className="p-6">
                        <div className="flex justify-between items-start">
                            <h3 className="text-xl font-black text-gray-900 line-clamp-1">{formData.title || 'Product Title'}</h3>
                            <span className="text-2xl font-black text-indigo-600">₹{formData.price || '0'}</span>
                        </div>
