        ...
        <div className="flex space-x-4 mb-8 border-b border-gray-200 dark:border-gray-700 pb-1 overflow-x-auto">
            {['stats', 'users', 'items'].map((tab) => (
                <button
                    ...
                    className={`pb-3 px-4 text-sm font-bold uppercase tracking-wider transition-colors border-b-2 whitespace-nowrap ${
                        activeTab === tab 
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {u.isBanned ? (
                                                <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200">
                                                    {u.banExpiresAt ? `Suspended (7d)` : `Banned Forever`}
                                                </span>
                                            ) : (
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            {!u.isAdmin && (
                                                <div className="flex justify-end items-center space-x-2">
                                                    {u.isBanned ? (
                                                        <button 
                                                            onClick={() => handleBanUser(u._id, 'unban')} 
                                                            title="Unban User"
                                                            className="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 bg-green-100 dark:bg-green-900/30 p-2 rounded-lg"
                                                        >
                                                            <FaUnlock /> Unban
                                                        </button>
                                                    ) : (
                                                        <>
                                                            <button 
                                                                onClick={() => handleBanUser(u._id, 'temporary')} 
                                                                title="7 Day Ban"
                                                                className="text-orange-600 hover:text-orange-900 dark:text-orange-400 bg-orange-100 dark:bg-orange-900/30 p-2 rounded-lg"
                                                            >
                                                                <FaClock />
                                                            </button>
                                                            <button 
                                                                onClick={() => handleBanUser(u._id, 'permanent')} 
                                                                title="Permanent Ban"
                                                                className="text-red-600 hover:text-red-900 dark:text-red-400 bg-red-100 dark:bg-red-900/30 p-2 rounded-lg"
                                                            >
                                                                <FaBan />
                                                            </button>
                                                        </>
                                    ...
                                    <tr key={item._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button 
                                                ...
                                                className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 bg-red-50 dark:bg-red-900/10 px-3 py-1.5 rounded-lg border border-red-100 dark:border-red-900 hover:bg-red-100 dark:hover:bg-red-900/30 transition"
                                            >
                                                Delete
                                            </button>
                                        </td>
